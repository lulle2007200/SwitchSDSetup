...
if [[ -z $FixMbr ]]
	...
	if [[ $Android ]] && (( $Android==3 ))
		...
		for ((i=1;i<(${#Names[@]}-1);i++))
			...
			PartNo=${#Partitions[@]}
			if expr match "${Names[$i]}" "^vendor$"
				then
				declare VendorPartNo=$PartNo
			fi
			if [[ $NoFormat ]]
				then
		...
		PostPartCmds=("${PostPartCmds[@]}" "mount ${Device}${PartPrefix}${VendorPartNo} ./DataPartitionMount")
		PostPartCmds=("${PostPartCmds[@]}" "unzip -o \"./StartFiles/R2P.zip" -d "./DataPartitionMount\"")
		PostPartCmds=("${PostPartCmds[@]}" "umount ./DataPartitionMount")
